...
export const load: PageServerLoad = async ({ locals }) => {
	...
	await locals.db.insert(subjects).values(subject).onConflictDoNothing();
	const subjectParent = await locals.db
	...
	await locals.db.insert(olympiads).values(olympiad).onConflictDoNothing();
	const olympiadParent = await locals.db
		.select({ id: olympiads.id })
		.from(olympiads)
		.where(eq(olympiads.name, 'IPhO'));
